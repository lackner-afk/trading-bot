"""
One Trading CCXT Datenfeed — Empfohlener Live-Feed für One Trading (Bitpanda Pro)

Basiert auf CCXT (offiziell unterstützt als 'onetrading').
Unterstützt:
- Öffentliche Marktdaten (ohne API-Key)
- Authentifizierte Calls (Balance, später Orders) mit API-Key

Symbol-Format: BTC_EUR (wie im Bot verwendet) ↔ BTC/EUR (CCXT)
"""
...
class OneTradingCCXTFeed:
    """
    One Trading Live-Datenfeed via CCXT.

    - Preis-Updates alle 5s via fetch_tickers()
    - Kerzen-Updates alle 60s via fetch_ohlcv()
    - Volle Kompatibilität mit KrakenFeed / OneTradingFeed Interface
    - Unterstützt API-Keys für private Endpoints (fetch_balance etc.)

    Verwendung:
        feed = OneTradingCCXTFeed(config={'pairs': ['BTC_EUR', 'ETH_EUR']})
        # oder mit Keys:
        feed = OneTradingCCXTFeed(
    ...
    def __init__(self, api_key: str = None, api_secret: str = None, config: Dict = None):
        ...
        # Symbol-Mapping: BTC_EUR → BTC/EUR (CCXT-Standard)
        ...
        if api_key and api_secret:
            ...
            self.logger.info("One Trading Feed mit API-Key initialisiert (authentifiziert)")
        else:
            self.logger.info("One Trading Feed ohne API-Key (nur öffentliche Daten)")

        self.exchange = ccxt.onetrading(exchange_config)
        ...
        # State
        self.running = False
        self._ws_connected = False  # Kompatibilität mit altem Interface
    ...
    async def start(self):
        """Startet den Feed"""
        self.running = True
        self.logger.info(f"Starte One Trading CCXT Feed für {self.pairs}")
    ...
    async def stop(self):
        ...
        self.logger.info("One Trading CCXT Feed gestoppt")
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        """Berechnet RSI, BB, EMA, VWAP (identisch zu KrakenFeed)"""
        if len(df) < 20:
            return df
        df = df.copy()
    ...
    # ===== Authentifizierte Methoden (für Live-Trading) =====

    async def get_balance(self) -> Optional[Dict]:
        """
        Holt den aktuellen Kontostand (nur mit gültigem API-Key).
        Gibt ein Dict mit 'free', 'used', 'total' pro Währung zurück.
        """
        if not self.api_key or not self.api_secret:
            self.logger.warning("get_balance() aufgerufen ohne API-Key")
            return None
        try:
            return await self.exchange.fetch_balance()
        except Exception as e:
            self.logger.error(f"Fehler beim Abrufen des Balances: {e}")
            return None

    async def get_open_orders(self, symbol: str = None) -> List[Dict]:
        """Holt offene Orders (für spätere Reconciliation)."""
        if not self.api_key:
            return []
        try:
            ccxt_symbol = self._to_ccxt.get(symbol) if symbol else None
            return await self.exchange.fetch_open_orders(ccxt_symbol)
        except Exception as e:
            self.logger.error(f"Fehler beim Abrufen offener Orders: {e}")
            return []

    def get_exchange(self) -> ccxt.onetrading:
        """Gibt die rohe CCXT Exchange-Instanz zurück (für fortgeschrittene Nutzung)."""
